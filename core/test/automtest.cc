...
BOOST_FIXTURE_TEST_SUITE(test_suite, omniNamesFixture)
...
BOOST_AUTO_TEST_CASE(getHost)
{
  ...
  std::cout << "Got host: " << ORBMgr::getMgr()->getHost(ior) << std::endl;
  BOOST_REQUIRE(ORBMgr::getMgr()!=0);
...
BOOST_AUTO_TEST_CASE(getPort)
{
  ...
  std::cout << "Got port: " << ORBMgr::getMgr()->getPort(ior) << std::endl;
  BOOST_REQUIRE(ORBMgr::getMgr()!=0);
...
BOOST_AUTO_TEST_CASE(getTypeID)
{
  ...
  std::cout << "Got TypeID: " << ORBMgr::getMgr()->getTypeID(ior) << std::endl;
  BOOST_REQUIRE(ORBMgr::getMgr()!=0);
...
BOOST_AUTO_TEST_CASE(listctxtobj)
{
  ...
  std::cout << "Nb objects in testORBT got: " << ORBMgr::getMgr()->list("dietAgent").size() << std::endl;
  BOOST_REQUIRE(ORBMgr::getMgr()!=0);
