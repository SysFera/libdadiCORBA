  ...
  tag_list_t& initialConfig)
{
  ...
  if (isComponentExists((const char*)componentName, it)) {
    ...
    if (!lost) {
      //      TRACE_TEXT(TRACE_MAIN_STEPS, "Connection of component '" << componentName << "' failed, "
  ...
  initialConfig = *tl;
  //  TRACE_TEXT(TRACE_MAIN_STEPS, "Connection of component '" << componentName << "' with message"
  //             << " '" << message << "'" << endl);
  return LS_OK;
}
                                              ...
                                              const char* message)
{
  ...
  if (!found) {
    delete it;
    //    TRACE_TEXT(TRACE_MAIN_STEPS, "Disconnection of component '" << componentName << "' failed,"
  ...
  //  TRACE_TEXT(TRACE_MAIN_STEPS, "Disconnection of '" << componentName << "' with message"
  //             << " '" << message << "'" << endl);
  return LS_OK;
}
...
LogCentralComponent_impl::sendBuffer(const log_msg_buf_t& buffer)
{
  ...
  if (buffer.length() != 0) {
    ...
    if (compExists == false) {
      //      TRACE_TEXT(TRACE_MAIN_STEPS, "Discarded messageBuffer from unknown component " << name << endl);
      return;
    }
...
LogCentralComponent_impl::AliveCheckThread::run_undetached(void* params)
{
  ...
  while (this->threadRunning) {
	...
	//        TRACE_TEXT(TRACE_MAIN_STEPS, "Ping Timeout of '" << s << "': disconnect the component." << endl);
...
LogCentralComponentFwdrImpl::LogCentralComponentFwdrImpl(CorbaLogForwarder_ptr fwdr,
							 const char* objName)
{
...
ComponentConfiguratorFwdr_impl::ComponentConfiguratorFwdr_impl(CorbaLogForwarder_ptr fwdr,
							       const char* objName){
  this->forwarder = CorbaLogForwarder::_duplicate(fwdr);
  this->objName = CORBA::string_dup(objName);
}
