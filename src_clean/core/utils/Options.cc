...
Options::processOptions() {
  ...
  for (l = flags.begin(); l != flags.end(); ++l) {
    if (flagCallbacks.find(*l) != flagCallbacks.end()) {
      ...
      if (*l != '-') {
        mlogger->log(dadi::Message("Options",
                                   "-" + std::string(&(*l)) + " flag unknown",
...
ConfigFile::parseFile(const std::string &path) {
  ...
  while (!file.eof()) {
    ...
    if (attributes.find(key) != attributes.end()) {
      ...
      os << l;
      mlogger->log(dadi::Message("Options",
                                 "Multiple values for the attribute " + key
                                 + " (l." + os.str() + ")",
                                 dadi::Message::PRIO_DEBUG));
    }
